...
configure() {
  ...
  for hwmon in $hwmon_glob; do
    ...
    if [[ "$hwmon_path" == "$fan_hwmon_pattern" ]]; then
      fan_hwmon_path="${hwmon_path}/pwm1"
    fi
  done
}
...
get_temps() {
    CPU_TEMP=`cut -c1-2 /sys/class/hwmon/hwmon0/temp1_input` 
    RAM_TEMP=`cut -c1-2 /sys/class/hwmon/hwmon3/temp1_input` 
    WIFI_TEMP=`cut -c1-2 /sys/class/hwmon/hwmon2/temp1_input`
}
